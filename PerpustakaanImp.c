...
void cetakBuku(dataBuku *data)
{
    ...
    while (data != NULL)
    {
        printf("no. %d\nISBN\t: %s\nJudul\t: %s\nJenis\t: %s\nAuthor\t: %s\nJumlah\t: %d\nNIDN\t: %s\n\n", i, data->ISBN, data->JudulBuku, data->JenisBuku, data->Author, data->jumlah, data->NIDN);
        data = data->alamat;
        i++;
    }
}

void cetakDataDosen(dataDosen *data)
{
    int i = 1;
    while (data != NULL)
    {
        printf("no. %d\nNIDN\t\t: %s\nNama Dosen\t: %s\nNomor Telepon\t: %s\n\n", i, data->NIDN, data->nama, data->no_telp);
        data = data->alamat;
        i++;
    }
}
...
void cariNIDN(dataDosen *data, dataBuku *isi, char *cari)
{
    while (data != NULL)
    {
        if (strcmp(data->NIDN, cari) == 0)
        {
            ...
            printf("Jumlah buku didonasikan\t: %d\n", isi->jumlah);
        }
        data = data->alamat;
    }
}
